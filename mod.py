...
def classify_pixel_bayesian(cb, cr, skin_hist, non_skin_hist):
    # Calculate the likelihood ratio
    if non_skin_hist[cb, cr] == 0:
        return 1
    likelihood_ratio = skin_hist[cb, cr] / non_skin_hist[cb, cr]
    return 1 if likelihood_ratio > 1 else 0


def classify_image_bayesian(image, skin_hist, non_skin_hist):
    # Get the dimensions of the image
    height, width = image.shape[:2]

    # Create an empty image to store the classified pixels
    classified_img = np.zeros((height, width), dtype=np.uint8)

    # Iterate through each pixel in the image
    for i in range(height):
        for j in range(width):
            # Get the Cb and Cr values of the current pixel
            cb = image[i, j, 1]
            cr = image[i, j, 2]

            # Calculate the likelihood ratio
            skin_prob = skin_hist[cb, cr]
            non_skin_prob = non_skin_hist[cb, cr]

            # Handle the case where the non_skin_prob is zero to avoid division by zero
            if non_skin_prob == 0:
                classified_img[i, j] = 1
            else:
                likelihood_ratio = skin_prob / non_skin_prob
                classified_img[i, j] = 1 if likelihood_ratio > 1 else 0
...
def main():
    ...
    train_pred_masks = [classify_image_bayesian(img, skin_hist, non_skin_hist) for img in train_ycbcr_images]

    # Classify testing images
    test_pred_masks = [classify_image_bayesian(img, skin_hist, non_skin_hist) for img in test_ycbcr_images]
    ...
    specific_image = cv2.imread(r'C:\\Users\\RAFAT\\Desktop\\MV06\\Skin_small\\Test\\images\\unknown.jpg')
    specific_mask_path = r'C:\\Users\\RAFAT\\Desktop\\MV06\\Skin_small\\Test\\masks\\unknown.png'
    ...
    specific_pred_mask = classify_image_bayesian(specific_image_ycbcr, skin_hist, non_skin_hist)
